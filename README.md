...
A local browser workspace for running several agent CLIs and shells in one saved, resizable deck.
...
- **New** creates a deck with one terminal using your `$SHELL` configuration.
...
Saved decks are written to `data/decks.json`. The server only listens on `127.0.0.1`.

## Privacy

Saved deck data can contain local paths, foreground commands, and agent session identifiers. `data/decks.json` is excluded from Git; keep it local and review it before sharing diagnostics or archives.
